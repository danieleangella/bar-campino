<script>
  ...
  onMount(async () => {
    ...
    try {
      const res = await fetch(`${PUBLIC_BASE_URL}/api/collections/events/records/${eventId}`);
      ...
      const shiftsRes = await fetch(
        `${PUBLIC_BASE_URL}/api/collections/shifts/records?filter=event%3D"${eventId}"&sort=%2Bstart_time&perPage=200`
      );
      const shiftsData = await shiftsRes.json();
      shifts = shiftsData.items.map(s => ({ id: s.id, name: s.name, start_time: s.start_time ?? '', end_time: s.end_time ?? '', min_volunteers: s.min_volunteers ?? 3 }));
    } catch (e) {
  ...
  async function save() {
    ...
    try {
      ...
      for (const id of deletedShifts) {
        await fetch(`${PUBLIC_BASE_URL}/api/collections/shifts/records/${id}`, { method: 'DELETE' });
      }
      for (const shift of shifts) {
        if (!shift.name) continue;
        if (shift.id) {
          await fetch(`${PUBLIC_BASE_URL}/api/collections/shifts/records/${shift.id}`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name: shift.name, start_time: shift.start_time, end_time: shift.end_time, min_volunteers: shift.min_volunteers })
          });
        } else {
...
<svelte:head><title>Modifica evento — Bar Campino</title></svelte:head>

<div style="min-height: 100vh; background: {auth ? 'var(--sfondo-auth)' : 'var(--sfondo)'};">
  <Header titolo="Modifica evento" auth={auth} token={token} linkIndietro="/admin?token={token}" testoIndietro="← Admin" />
        ...
        <div style="background: var(--sfondo-card); border: 1px solid var(--bordo);" class="rounded-2xl p-5">
          ...
          <div class="flex flex-col gap-4">
            <div>
              ...
              <input type="text" bind:value={form.title} style="background: var(--sfondo-input); border: 1px solid var(--bordo); color: var(--testo);" class="w-full rounded-lg px-3 py-2" />
            </div>
            <div>
            ...
            <label class="flex items-center gap-3 cursor-pointer">
              ...
              <span class="text-sm">Pubblicato</span>
            </label>
          </div>
        </div>
        <div style="background: var(--sfondo-card); border: 1px solid var(--bordo);" class="rounded-2xl p-5">
              ...
              <div style="background: var(--sfondo-input); border: 1px solid var(--bordo);" class="rounded-xl p-4">
                ...
                <div class="flex flex-col gap-3">
                  <div>
                    ...
                    <input type="text" bind:value={shift.name} style="background: var(--sfondo-card); border: 1px solid var(--bordo); color: var(--testo);" class="w-full rounded-lg px-3 py-2" />
                  </div>
